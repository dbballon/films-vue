    ...
    >
      <b-form>
        ...
        <b-form-group label="Year" label-for="year" class="text-light">
          <b-form-input id="year" type="text" v-model="form.year" placeholder="Year"></b-form-input>
...
export default {
  ...
  methods: {
    ...
    validateForm() {
      ...
      Object.keys(this.errorFields).map(key => {
        errorFields[key] = "";
      });
      let valid = true;
      if (this.form.title == "") {
      ...
      if (this.form.year == "") {
        ...
        Number(this.form.year) < 1860 ||
        Number(this.form.year) > 2050
      ) {
